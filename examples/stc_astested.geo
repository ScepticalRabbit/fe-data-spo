...
file_name = "stc_astested.msh";

//-----------------------------------------
// MEASURED FROM CAD
// armour above fillet = 14mm
// fillet = 2mm
// pipe thick = 1.5mm
// pipe rad inner = 6mm
// dist fillet base to bottom = 2mm
// total block height = 35mm
//
// pipe cent to top of block = 23.5mm
// pipe cent to base of block = 11.5mm
//-----------------------------------------

// Geometric variables
block_width = 37e-3;    // correct
block_leng = 50e-3;     // correct
//block_armour = 12e-3;
block_height_below_pipe = 11.5e-3;
block_height_above_pipe = 23.5e-3;
block_height_tot = block_height_below_pipe+block_height_above_pipe; //should be 35 mm

// Block half width must be greater than the sum of:
// block_width/2 >= pipe_rad_in+pipe_thick_fillet_rad
fillet_rad = 2e-3;      // correct
pipe_rad_in = 6e-3;     // correct
pipe_thick = 1.5e-3;    // correct
...
pipe_loc_x = 0.0;
pipe_loc_y = block_height_below_pipe;
...
ps1() = Surface In BoundingBox{
    ...
    block_width/2+tol,0.0+tol,block_leng/2+tol};
Physical Surface("bc-base-surf") = {ps1(0),ps1(1)};
...
// Radiative BCs for surfaces
ps4() = Surface In BoundingBox{
    -block_width/2-tol,0.0-tol,-block_leng/2-tol,
    block_width/2+tol,block_height_tot+tol,-block_leng/2+tol};
Physical Surface("bc-left-surf") = {ps4(0),ps4(1)};

ps5() = Surface In BoundingBox{
    -block_width/2-tol,0.0-tol,block_leng/2-tol,
    block_width/2+tol,block_height_tot+tol,block_leng/2+tol};
Physical Surface("bc-right-surf") = {ps5(0),ps5(1)};

ps6() = Surface In BoundingBox{
    block_width/2-tol,0.0-tol,-block_leng/2-tol,
    block_width/2+tol,block_height_tot+tol,block_leng/2+tol};
Physical Surface("bc-front-surf") = {ps6(0),ps6(1)};

ps7() = Surface In BoundingBox{
    -block_width/2-tol,0.0-tol,-block_leng/2-tol,
    -block_width/2+tol,block_height_tot+tol,block_leng/2+tol};
Physical Surface("bc-back-surf") = {ps7(0),ps7(1)};
